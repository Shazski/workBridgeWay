                    ...
                    <tbody>
                      {
                        filteredData.slice(firstIndex, lastIndex).map((applicant, idx) => {
                          return <tr key={idx}
                            ...
                            <td className="whitespace-nowrap px-6 py-4"><h1 className={`mt-3 border-2 text-${statusColor[applicant?.hiringStage]}-600 py-1 px-3 w-min rounded-2xl uppercase font-bold  border-${statusColor[applicant?.hiringStage]}-600 `}>{applicant?.hiringStage}</h1></td>
