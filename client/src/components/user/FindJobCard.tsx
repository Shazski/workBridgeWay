    ...
    return (
        <div className="border md:flex md:h-28 w-full md:w-full mt-8 md:justify-center">
            ...
            <div className="lg:ms-24 ms-2">
                <button onClick={() => setIsModalOpen(true)} disabled={jobs?.vacancy <= jobs?.applicants?.length || jobs?.applicants?.some(applicant => applicant?.applicantId?.toString() === user?._id?.toString())} className="bg-lightgreen disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold px-10 ms-5 py-2 md:mt-6 me-4 rounded-sm">{jobs?.applicants?.some(applicant => applicant?.applicantId?.toString() === user?._id?.toString()) ? "Applied" : "Apply"}</button>
                {
