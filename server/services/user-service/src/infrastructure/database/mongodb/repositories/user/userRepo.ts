...
export const findUserByEmail_repo = async (
  ...
  try {
    ...
    if(userCredentials.phone) {
       userExists = await UserSchema.findOne({$or:[{email:userCredentials.email},{phone:userCredentials.phone}]});
    } else {
       userExists = await UserSchema.findOne({$or:[{email:userCredentials.email}]});
    }
